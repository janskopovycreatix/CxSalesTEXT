...
begin
  ...
  end else FCached := (NativeInt(FBuffer) = FDataSet.ActiveBuffer);
  FOpened := True;
...
begin
  ...
  begin
    cxCopyData(PByte(ActiveBuffer), Buffer, RecordSize);
    Result := True;
  end
  else
    Result := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        if FData.GetActiveBuffer(PByte(ActiveBuffer), ABuf, FSortedField) then
    ...
    FData.SetBuffer(PByte(ActiveBuffer), FCurRec);
  end else
  begin
    Inc(FLastBookmark);
    ...
    FData.InsertRecord(PByte(ActiveBuffer), FCurRec, False);
    FBookMarks.Add(TValueBuffer(FLastBookmark));
    InternalAddFilterRecord;
  end;

  if BlobFieldCount > 0 then
    SetMemBlobData(PByte(ActiveBuffer));
...
begin
  ...
  begin
    ...
    if (mField <> nil) then
    begin
      buf := mField.GetDataFromBuffer(PByte(ActiveBuffer));
...
begin
  ...
  FData.InsertRecord(PByte(ActiveBuffer), FCurRec, Append);
  FBookMarks.Add(TValueBuffer(FLastBookmark));
...
var
  ...
  AC84207EA9266188A9EAFD70E2D5A8238: TSQLTimeStamp;
begin
  case AField.DataType of
    ftString, ftGuid:
    ...
    ftTimeStamp:
    begin
      AC84207EA9266188A9EAFD70E2D5A8238 := GetTimeStampValue(ABuffer, AField);
      Result := VarSQLTimeStampCreate(AC84207EA9266188A9EAFD70E2D5A8238);
    end;
    ftBCD:
    begin
...
begin
  if (FBlobList <> nil) and (FCurRec >= 0) and (FCurRec < RecordCount) and (State in dsEditModes) then
    SetBlobData(PByte(ActiveBuffer), Field, GetBlobData(PByte(FBlobList[FCurRec]), Field.Offset))
  else
    SetBlobData(PByte(ActiveBuffer), Field, '');
end;
